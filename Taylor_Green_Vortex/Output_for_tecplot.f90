...
!     !----------------------------------------------------------------------------------------------------------
!     !Zone for water
!     write(2,*) 'ZONE T=" '//trim(adjustl(char_time))//' ",'//' I=',particle_ture_number," F=POINT"
!     !Time shoule be in quotation marks ("")
    
!     do i=1,particle_ture_number
!        write(2,100) (particle_position(i,j)/domain_size_y,j=1,dim),(particle_velocity(i,j)/velocity_0,j=1,dim),particle_press(i)/pressure_0
! 100    format(6F20.8)   
!     end do
!     !----------------------------------------------------------------------------------------------------------
...
!     write(2,*) 'ZONE T=" '//trim(adjustl(char_time))//' ",'//' I=',wave_maker_particle_number," F=POINT"
!     !Time shoule be in quotation marks ("")
...
!     write(2,*) 'ZONE T=" '//trim(adjustl(char_time))//' ",'//' I=',fix_ghost_particle_number," F=POINT"
!     !Time shoule be in quotation marks ("")
...
!     !==========================================================================================================
    ...
    write(2,*) 'ZONE T=" '//trim(adjustl(char_time))//' ",'//' I=',ture_total_particle_number," F=POINT"
!     !Time shoule be in quotation marks ("")
    
    do i=1,ture_total_particle_number
       Velocity_Module=sqrt(DOT_PRODUCT(particle_velocity(i,:),particle_velocity(i,:)))
       write(2,100) (particle_position(i,j)/domain_size_y,j=1,dim),(particle_velocity(i,j)/velocity_0,j=1,dim),particle_press(i)/pressure_0,Velocity_Module/velocity_0,particle_vorticity(i,3)*time_0,(particle_velocity_error(i,j),j=1,dim),particle_press_error(i),particle_velocity_magnitude_error(i)
100    format(20F20.8)   
    end do
    !----------------------------------------------------------------------------------------------------------
    ...
    write(10,*) 'ZONE T=" '//trim(adjustl(char_time))//' ",'//' I=',surface_particle_number," F=POINT"
!     !Time shoule be in quotation marks ("")

    ! do i=1,particle_ture_number
    
    !     if (free_surface_type(i)==1) then
    !        write(10,100) (particle_position(i,j)/domain_size_y,j=1,dim),(Normal_vector(i,j),j=1,dim)
    !     end if
    ! end do
    ...
    if(dirExists) then
        ...
        write(Binary_Tecplot_Port) 6                                      !Variable number
        ...
        !---------IMax,JMax,KMax
        write(Binary_Tecplot_Port) ture_total_particle_number,1,1                              !(IMax)
        ...
        do i=1,ture_total_particle_number
           Velocity_Module=sqrt(particle_velocity(i,1)**2+particle_velocity(i,2)**2)
           write(Binary_Tecplot_Port) (particle_position(i,j)/domain_size_y,j=1,dim),(particle_velocity(i,j)/velocity_0,j=1,dim),particle_press(i)/pressure_0,Velocity_Module/velocity_0   
        end do
