...
!****************************************************************************
 ...
 subroutine Solver_Normalized_HLLC_Linear_Riemann_Delta_SPS()
    ...
    do k=1,pair_number
        ...
        sin_theta=IJ_Direction_Vector(2)
        cos_theta=IJ_Direction_Vector(1)

        Velocity_Left(1)=I_Velocity(1)*cos_theta + I_Velocity(2)*sin_theta
        Velocity_Left(2)=-I_Velocity(1)*sin_theta + I_Velocity(2)*cos_theta

        Velocity_Right(1)=J_Velocity(1)*cos_theta + J_Velocity(2)*sin_theta
        Velocity_Right(2)=-J_Velocity(1)*sin_theta + J_Velocity(2)*cos_theta
        ...
        !Pressure star
        P_asterisk=(P_Left*c_Left*rho_Left+P_Right*c_Right*rho_Right-c_Left*rho_Left*c_Right*rho_Right*(Velocity_Right(1)-Velocity_Left(1)))/(rho_Left*c_Left+rho_Right*c_Right)

        Rho_asterisk=water_rho_0*(1+7*P_asterisk/(square_c_0*water_rho_0))**(0.142857142)
        ...
        if ( P_asterisk <= P_Left ) then          !Rarefaction
          ...
          temp=((rho_Left/Rho_asterisk_Left)*(P_Left-P_asterisk))/((rho_Left-Rho_asterisk_Left)*c_Left**2)
        ...
        if ( P_asterisk <= P_Right ) then         !Rarefaction
          ...
          temp=((rho_Right/Rho_asterisk_Right)*(P_Right-P_asterisk))/((rho_Right-Rho_asterisk_Right)*c_Right**2)
        ...
        velocity_asterisk(1)=Velocity_HLLC(1)*cos_theta - Velocity_HLLC(2)*sin_theta
        velocity_asterisk(2)=Velocity_HLLC(1)*sin_theta + Velocity_HLLC(2)*cos_theta
        !P_asterisk=square_c_0*(Rho_HLLC-water_rho_0) 
 ...
 end subroutine Solver_Normalized_HLLC_Linear_Riemann_Delta_SPS