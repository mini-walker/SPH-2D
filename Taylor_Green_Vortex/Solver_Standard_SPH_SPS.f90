...
!
!  SUBROUTINE: Solver_Standard_SPH_SPS
!
!  PURPOSE: Compute iterator information for time integration with SPS
!
...
!****************************************************************************
 ...
 subroutine Solver_Standard_SPH_SPS()
    ...
    do k=1,pair_number
       ...
       !velocity difference (Attention: j-i)
        velocity_difference(:)=-velocity_difference_ij(k,:)     
       
       !Calculate SPS Strain Tensor
       if(dim==1) then
           SPS_Strain_Tensor_xx(i)=SPS_Strain_Tensor_xx(i)+velocity_difference(1)*dwdx(k,1)*subdomain_particle_volume(j)
           SPS_Strain_Tensor_xx(j)=SPS_Strain_Tensor_xx(j)+velocity_difference(1)*dwdx(k,1)*subdomain_particle_volume(i)
       else if(dim==2) then
           SPS_Strain_Tensor_xx(i)=SPS_Strain_Tensor_xx(i)+velocity_difference(1)*dwdx(k,1)*subdomain_particle_volume(j)
           SPS_Strain_Tensor_xy(i)=SPS_Strain_Tensor_xy(i)+(velocity_difference(1)*dwdx(k,2)+velocity_difference(2)*dwdx(k,1))*subdomain_particle_volume(j)
           SPS_Strain_Tensor_yy(i)=SPS_Strain_Tensor_yy(i)+velocity_difference(2)*dwdx(k,2)*subdomain_particle_volume(j)

           SPS_Strain_Tensor_xx(j)=SPS_Strain_Tensor_xx(j)+velocity_difference(1)*dwdx(k,1)*subdomain_particle_volume(i)
           SPS_Strain_Tensor_xy(j)=SPS_Strain_Tensor_xy(j)+(velocity_difference(1)*dwdx(k,2)+velocity_difference(2)*dwdx(k,1))*subdomain_particle_volume(i)
           SPS_Strain_Tensor_yy(j)=SPS_Strain_Tensor_yy(j)+velocity_difference(2)*dwdx(k,2)*subdomain_particle_volume(i)
       else if(dim==3) then
           SPS_Strain_Tensor_xx(i)=SPS_Strain_Tensor_xx(i)+velocity_difference(1)*dwdx(k,1)*subdomain_particle_volume(j)
           SPS_Strain_Tensor_xy(i)=SPS_Strain_Tensor_xy(i)+(velocity_difference(1)*dwdx(k,2)+velocity_difference(2)*dwdx(k,1))*subdomain_particle_volume(j)
           SPS_Strain_Tensor_xz(i)=SPS_Strain_Tensor_xz(i)+(velocity_difference(1)*dwdx(k,3)+velocity_difference(3)*dwdx(k,1))*subdomain_particle_volume(j)
           SPS_Strain_Tensor_yy(i)=SPS_Strain_Tensor_yy(i)+velocity_difference(2)*dwdx(k,2)*subdomain_particle_volume(j)
           SPS_Strain_Tensor_yz(i)=SPS_Strain_Tensor_yz(i)+(velocity_difference(2)*dwdx(k,3)+velocity_difference(3)*dwdx(k,2))*subdomain_particle_volume(j)
           SPS_Strain_Tensor_zz(i)=SPS_Strain_Tensor_zz(i)+velocity_difference(3)*dwdx(k,3)*subdomain_particle_volume(j)

           SPS_Strain_Tensor_xx(j)=SPS_Strain_Tensor_xx(j)+velocity_difference(1)*dwdx(k,1)*subdomain_particle_volume(i)
           SPS_Strain_Tensor_xy(j)=SPS_Strain_Tensor_xy(j)+(velocity_difference(1)*dwdx(k,2)+velocity_difference(2)*dwdx(k,1))*subdomain_particle_volume(i)
           SPS_Strain_Tensor_xz(j)=SPS_Strain_Tensor_xz(j)+(velocity_difference(1)*dwdx(k,3)+velocity_difference(3)*dwdx(k,1))*subdomain_particle_volume(i)
           SPS_Strain_Tensor_yy(j)=SPS_Strain_Tensor_yy(j)+velocity_difference(2)*dwdx(k,2)*subdomain_particle_volume(i)
           SPS_Strain_Tensor_yz(j)=SPS_Strain_Tensor_yz(j)+(velocity_difference(2)*dwdx(k,3)+velocity_difference(3)*dwdx(k,2))*subdomain_particle_volume(i)
           SPS_Strain_Tensor_zz(j)=SPS_Strain_Tensor_zz(j)+velocity_difference(3)*dwdx(k,3)*subdomain_particle_volume(i)
       else
 ...
 end subroutine Solver_Standard_SPH_SPS