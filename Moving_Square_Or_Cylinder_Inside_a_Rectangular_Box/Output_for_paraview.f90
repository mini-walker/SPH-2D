...
subroutine Output_for_paraview(i_time_step)
    ...
    !Variables for paraview binary output
    ...
    ASCII_Paraview:if(dirExists) then
      ...
      open(unit=Paraview_General_File_Port,file=trim(adjustl(File_name)),status="replace",position="rewind",action="write",iostat=ioerror)
      !--------------------------------------------------------------------------------------------------------
      ...
      ! !Output the data
      ! !Header of the file
      ! write (Paraview_General_File_Port,'(a)') '<?xml version="1.0"?>'
      ! write (Paraview_General_File_Port,'(a)') '<VTKFile type="UnstructuredGrid" version="0.1" byte_order="BigEndian">' 
      ! write (Paraview_General_File_Port,'(a)') '<UnstructuredGrid>'

      ! !Point number
      ! write (Paraview_General_File_Port,*) '<Piece NumberOfPoints="',ture_total_particle_number,'" NumberOfCells="',ture_total_particle_number,'" >'

      ! !--------------------------------------------------------------------------------------------------------
      ! !Velocity
      ! write (Paraview_General_File_Port,'(a)') '<PointData>'
      ! write (Paraview_General_File_Port,'(a)') '<DataArray type="Float64" Name="Velocity" NumberOfComponents="3" format="ascii">'
      ! if (dim==2) then
          
      !     do i = 1,ture_total_particle_number
      !        write (Paraview_General_File_Port,"(101E20.9E3)") (particle_velocity(i,j)/velocity_0,j=1,dim),0.0000
      !     enddo

      ! else if (dim==3) then

      !     do i = 1,ture_total_particle_number
      !        write (Paraview_General_File_Port,"(101E20.9E3)") (particle_velocity(i,j)/velocity_0,j=1,dim)
      !     enddo
        
      ! endif
      ! write (Paraview_General_File_Port,'(a)') '</DataArray>'
      ! !--------------------------------------------------------------------------------------------------------


      ! !--------------------------------------------------------------------------------------------------------
      ! !Pressure
      ! write (Paraview_General_File_Port,'(a)') '<DataArray type="Float64" Name="Pressure" format="ascii">'

      ! do i = 1,ture_total_particle_number
      !    write (Paraview_General_File_Port,"(101E20.9E3)") particle_press(i)/pressure_0
      ! enddo
      ! write (Paraview_General_File_Port,'(a)') '</DataArray>'
      ! !--------------------------------------------------------------------------------------------------------
      ...
      ! !Position
      ! write (Paraview_General_File_Port,'(a)') '</PointData>'
      ! write (Paraview_General_File_Port,'(a)') '<Points>'
      ! write (Paraview_General_File_Port,'(a)') '<DataArray type="Float64" NumberOfComponents="3" format="ascii">'

      ! if (dim==2) then
          
      !     do i = 1,ture_total_particle_number
      !        write (Paraview_General_File_Port,"(101E20.9E3)") (particle_position(i,j)/NonDimension_Reference_Length,j=1,dim),0.0000
      !     enddo

      ! else if (dim==3) then

      !     do i = 1,ture_total_particle_number
      !        write (Paraview_General_File_Port,"(101E20.9E3)") (particle_position(i,j)/NonDimension_Reference_Length,j=1,dim)
      !     enddo
      ...
      ! !--------------------------------------------------------------------------------------------------------
      ! !End of the file
      ! write (Paraview_General_File_Port,'(a)') '<Cells>'
      ! write (Paraview_General_File_Port,'(a)') '<DataArray type="Int32" Name="connectivity" format="ascii">'
      ! do i = 1,ture_total_particle_number
      !    write (Paraview_General_File_Port,"(I5)") i + 1 
      ! enddo
      ! write (Paraview_General_File_Port,'(a)') '</DataArray>'

      ! write (Paraview_General_File_Port,'(a)') '<DataArray type="Int32" Name="offsets" format="ascii">'
      ! do i = 1,ture_total_particle_number
      !    write (Paraview_General_File_Port,"(I5)") 1 
      ! enddo
      ! write (Paraview_General_File_Port,'(a)') '</DataArray>'

      ! write (Paraview_General_File_Port,'(a)') '<DataArray type="Int32" Name="types" format="ascii">'
      ! do i = 1,ture_total_particle_number
      !    write (Paraview_General_File_Port,"(I5)") 1 
      ! enddo
      ! write (Paraview_General_File_Port,'(a)') '</DataArray>'
      ...
      ! write (Paraview_General_File_Port,'(a)') '</Piece>'
      ! write (Paraview_General_File_Port,'(a)') '</UnstructuredGrid>'
      ! write (Paraview_General_File_Port,'(a)') '</VTKFile>'
      ! !--------------------------------------------------------------------------------------------------------
  
      ! close(Paraview_General_File_Port)
      ! !========================================================================================================
      ...
      !Point number
      write (Paraview_General_File_Port,*) '<Piece NumberOfPoints="',ture_total_particle_number,'" NumberOfCells="',0,'" >'
      
      !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
      write (Paraview_General_File_Port,'(a)') '<Points>'          !Points position
      !Data array type (Position)
      write (Paraview_General_File_Port,'(a)') '<DataArray name="Position" type="Float64" NumberOfComponents="3" format="ascii">'
      ...
      write (Paraview_General_File_Port,'(a)') '</DataArray>'
      write (Paraview_General_File_Port,'(a)') '</Points>'         !End Points position
      !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
     
      !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
      !Point data
      !Velocity
      write (Paraview_General_File_Port,'(a)') '<PointData Scalars="Pressure" Vectors="Velocity" >'       !Data at points
      ...
      do i = 1,ture_total_particle_number
         write (Paraview_General_File_Port,"(101E20.9E3)") (particle_press(i)-Background_Pressure)/pressure_0
      enddo
      write (Paraview_General_File_Port,'(a)') '</DataArray>'
      !--------------------------------------------------------------------------------------------------------

      write (Paraview_General_File_Port,'(a)') '</PointData>'                      !End Points Data type
      !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
      ...
      write (Paraview_General_File_Port,'(a)') '<DataArray type="Int32" Name="offsets" format="ascii">'
      write (Paraview_General_File_Port,'(a)') '</DataArray>'
      write (Paraview_General_File_Port,'(a)') '<DataArray type="UInt8" Name="types" format="ascii">'
      write (Paraview_General_File_Port,'(a)') '</DataArray>'
      write (Paraview_General_File_Port,'(a)') '</Cells>'
      write (Paraview_General_File_Port,'(a)') '</Piece>'
      write (Paraview_General_File_Port,'(a)') '</UnstructuredGrid>'
    ...
    !   Binary_Paraview_Port=Paraview_General_File_Port
    ...
    !       do i = 1,ture_total_particle_number
    !          write (Binary_Paraview_Port) (particle_position(i,j)/NonDimension_Reference_Length,j=1,dim),double_float
    !       enddo

    !   else if (dim==3) then

    !       do i = 1,ture_total_particle_number
    !          write (Binary_Paraview_Port) (particle_position(i,j)/NonDimension_Reference_Length,j=1,dim)
    !       enddo
